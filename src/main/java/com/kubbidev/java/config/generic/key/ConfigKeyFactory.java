package com.kubbidev.java.config.generic.key;
...
public interface ConfigKeyFactory<T> {
    ...
    ConfigKeyFactory<Map<String, String>> STRING_MAP = ConfigurationAdapter::getStringMap;

    ConfigKeyFactory<String[]> STRING_ARRAY = (adapter, path, def) -> adapter.getStringList(path, Arrays.asList(def)).toArray(new String[0]);

    ConfigKeyFactory<String> LOWERCASE_STRING = (adapter, path, def) -> adapter.getString(path, def).toLowerCase(Locale.ROOT);
    ConfigKeyFactory<String> UPPERCASE_STRING = (adapter, path, def) -> adapter.getString(path, def).toUpperCase(Locale.ROOT);
    ...
    static SimpleConfigKey<Map<String, String>> stringMapKey(String path, Map<String, String> def) {
        return key(new Bound<>(STRING_MAP, path, def));
    }

    static SimpleConfigKey<String[]> stringArrayKey(String path, String[] def) {
        return key(new Bound<>(STRING_ARRAY, path, def));
    }
    ...
    static SimpleConfigKey<String> uppercaseStringKey(String path, String def) {
        return key(new Bound<>(UPPERCASE_STRING, path, def));
    }

    static <E extends Enum<E>> SimpleConfigKey<E> enumKey(String path, E def) {
        return key(new Bound<>(ConfigurationAdapter::getEnum, path, def));
    }
